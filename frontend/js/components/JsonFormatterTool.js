  ...
  template: `
    <div class="container mx-auto px-4 py-8">
      <div class="flex items-center mb-6">
        ...
        <div>
          <h2 class="text-2xl font-bold">JSON格式化</h2>
          <p class="text-[#94A3B8]">格式化并验证JSON数据，支持缩进和压缩</p>
        </div>
      </div>
...
• Ctrl+L: 加载示例"
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div 
            ...
            :key="tool.id"
            class="card p-4 hover:bg-[#383850] cursor-pointer"
            @click="$router.push('/tool/' + tool.id)"
          >
            <i :class="tool.icon + ' text-[#165DFF] mr-2'"></i>
            {{ tool.name }}
          </div>
        </div>
      </div>
  ...
  setup() {
    ...
    const relatedTools = computed(() => {
      // 获取格式化工具分类中的其他工具
      const formatCategory = toolsConfig.categories.find(cat => cat.id === 'format');
      return formatCategory ? formatCategory.tools.filter(tool => tool.id !== 'json-formatter') : [];
    });
